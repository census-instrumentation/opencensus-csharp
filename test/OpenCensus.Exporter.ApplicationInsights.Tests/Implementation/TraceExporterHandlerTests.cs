...
namespace OpenCensus.Exporter.ApplicationInsights.Tests
{
    ...
    public class OpenCensusTelemetryConverterTests
    {
        ...
        /*
                ...
                {
                    var span = this.CreateBasicSpan(SpanKind.Unspecified, "spanName");
                ...
                private IAttributeValue CreateAttributeValue<T>(T value)
                {
                    if (value is int)
                    {
                        return AttributeValue.LongAttributeValue(Convert.ToInt32(value));
                    }

                    if (value is long)
                    {
                        return AttributeValue.LongAttributeValue(Convert.ToInt64(value));
                    }

                    if (value is bool)
                    {
                        return AttributeValue.BooleanAttributeValue(Convert.ToBoolean(value));
                    }

                    var s = value as string;
                    if (s != null)
                    {
                        return AttributeValue.StringAttributeValue(s);
                    }

                    return AttributeValue.StringAttributeValue(value.ToString());
                }
